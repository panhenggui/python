...
class APIKeyAuthenticator(Authenticator):
    ...
    def generate_signature(self, secret, verb, url, nonce, data):
        ...
        print("\ngenerate_signature data: " + str(data))

        parsedURL = urllib.parse.urlparse(url)
        print("\ngenerate_signature parsedURL: " + str(parsedURL))

        path = parsedURL.path
        if parsedURL.query:
            path = path + '?' + parsedURL.query
            print("\ngenerate_signature path: " + path)

        if data:
            message = bytes(verb + path + str(nonce) + data).encode('utf-8')
        else:
            message = verb + path + str(nonce)
        # message added by chaiyu 2019-04-01
        print("\nComputing HMAC: %s" % message)

        signature = hmac.new(bytes(secret).encode('utf-8'), message, digestmod=hashlib.sha256).hexdigest()
